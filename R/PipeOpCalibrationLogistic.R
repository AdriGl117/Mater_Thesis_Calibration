...
PipeOpLogisticCalibration <- R6Class(
  ...
  public = list(
    ...
    initialize = function(id = "logistic_calibration", learner, calibration_ratio = 0.2) {
  ...
  private = list(
    .train = function(inputs) {
      ...
      # Predict calibration data on base learner
      preds = self$learner$predict(calibration_task)
      # Train the calibrator
      pred_data = as.data.table(preds)
      calibration_data = data.table(truth = calibration_task$truth(), response = pred_data$response)
      self$calibrator = glm(truth ~ ., data = calibration_data, family = binomial)

      return(list(NULL)) # PipeOp train method should return a list even if output is NULL
    },
    ...
    .predict = function(inputs) {
      ...
      calibrated_response = predict(self$calibrator, newdata = data.frame(response = pred_data$response), type = "response")

      # Create a new PredictionRegr object with calibrated predictions
      pred_calibrated = PredictionRegr$new(task = task, response = calibrated_response)
