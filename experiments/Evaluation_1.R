###### Evalutaion Script for Benchmark Experiment #######
source("sources.R")
...
res = res[, .(Learner, Calibrator, Resampling, task_id, task_size, classif.ece, 
              classif.bbrier, classif.logloss)]

##### Results per Resampling Strategie #####
# Pivot on Resampling
measures <- c("classif.ece", "classif.bbrier", "classif.logloss")
resampling_columns <- c("holdout_70", "holdout_80", "holdout_90", "cv_3", 
                        "cv_5", "union")
for (measure in measures) {
  # Creates a pivot table for the resampling strategies once for all measures, 
  # plots and save the CD plots, create average ranks and scores and save the
  # results in a combined data.table
  ...
  res_resamplings_pivot <- res_resamplings_pivot %>%
    select(-starts_with("uncalibrated"), -starts_with("Learner"),
           -starts_with("Calibrator"), -starts_with("task_id"))
  p <- cd_plot(res_resamplings_pivot) 
  ggsave(paste0("figures/Exp_1_Resampling_", measure, ".jpeg"), dpi = 300)
  res_resamplings_pivot <- res_resamplings_pivot[
    complete.cases(res_resamplings_pivot)]
  rank_matrix <- t(apply(as.data.frame(res_resamplings_pivot),
  ...
  colnames(df) = c("Resampling", paste0(measure,"_mean"), 
...
# Pivot in resampling, grouped by Task size
size_values <- c("small", "medium", "large")
for (measure in measures) {
  for (size_value in size_values) {
    # Creates a pivot table for the resampling strategies once for all measures 
    # and task size, plots and save the CD plots, create average ranks and 
    # scores and save the results in a combined data.table
...
# Pivot on Calibration Approach
measures <- c("classif.ece", "classif.bbrier", "classif.logloss")
calibrator_columns <- c("platt", "beta", "isotonic", "uncalibrated")
for (measure in measures) {
  # Creates a pivot table for the approaches once for all measures,
  # plots and save the CD plots, create average ranks and scores and save the
  # results in a combined data.table
  res_filtered <- res %>% filter(Resampling == "uncalibrated" | 
                                   Resampling == "cv_5")
  ...
  rank_matrix <- t(apply(as.data.frame(res_calibrators_pivot), 
                         1, rank, ties.method = "average"))
  rank_dt <- as.data.table(rank_matrix)
  setnames(rank_dt, paste0(names(rank_dt), "_rank"))
  ...
  colnames(df) = c("Calibrator", paste0(measure,"_mean"), 
                   paste0(measure,"_rank"))
...
# Pivot on calibration, grouped by Task size
size_values <- c("small", "medium", "large")
for (measure in measures) {
  for (size_value in size_values) {
    # Creates a pivot table for the calibration approaches once for all measures 
    # and task size, plots and save the CD plots, create average ranks and 
    # scores and save the results in a combined data.table
    res_filtered <- res %>% filter(task_size == size_value & 
    ...
    res_calibrator_pivot <- res_calibrator_pivot %>%
      select(-starts_with("Learner"), -starts_with("task_id"))
    res_calibrator_pivot <- res_calibrator_pivot[
      complete.cases(res_calibrator_pivot)]
    p <- cd_plot(res_calibrator_pivot)
    ggsave(paste0("figures/Exp_1_Calibrator_", 
                  size_value, "_", measure, ".jpeg"), dpi = 300)
    rank_matrix <- t(apply(as.data.frame(res_calibrator_pivot), 
                           1, rank, ties.method = "average"))
    rank_dt <- as.data.table(rank_matrix)
    setnames(rank_dt, paste0(names(rank_dt), "_rank"))
    ...
    colnames(df) = c("Size", "Calibrator", paste0(measure,"_mean"), 
                     paste0(measure,"_rank"))
    if(!exists("res_per_measure")) {
      res_per_measure <- df
    } else {
...
# Pivot on Resampling, gruped by Calibrator
measures <- c("classif.ece", "classif.bbrier", "classif.logloss")
resampling_columns <- c("holdout_70", "holdout_80", "holdout_90",
                        "cv_3", "cv_5", "union")
calibrator_columns <- c("platt", "beta", "isotonic")
for(measure in measures){
  for(calibrator_value in calibrator_columns){
    # Creates a pivot table for the resampling strategies once for all measures 
    # and calibratprs, plots and save the CD plots, create average ranks and 
    # scores and save the results in a combined data.table
    ...
    res_calibrator_pivot <- res_calibrator_pivot %>%
      select(-starts_with("Learner"), -starts_with("task_id"), 
             -starts_with("Calibrator"))
    p <- cd_plot(res_calibrator_pivot)
    ggsave(paste0("figures/Exp_1_Resamplings_", calibrator_value, 
                  "_", measure, ".jpeg"), dpi = 300)
    rank_matrix <- t(apply(as.data.frame(res_calibrator_pivot),
                           1, rank, ties.method = "average"))
    rank_dt <- as.data.table(rank_matrix)
    setnames(rank_dt, paste0(names(rank_dt), "_rank"))
    ...
    colnames(df) = c("Calibrator", "Resampling", paste0(measure,"_mean"), 
                     paste0(measure,"_rank"))
    if(!exists("res_per_measure")) {
      res_per_measure <- df
    } else {
    ...
    if(nrow(res_per_measure) == 18) {
      if (!exists("res_calibrator_resampling_grouped")) {
        ...
        res_calibrator_resampling_grouped  <- merge(
          res_calibrator_resampling_grouped, res_per_measure, 
...
# Pivot on Calibration, grouped by Learner
res_learner = res %>% 
...
for(measure in measures){
  for(learner_value in learner_values){
    # Creates a pivot table for the calibration approaches once for all measures 
    # and learners, create average ranks and scores and save the results in a 
    # combined data.table
    ...
    res_learner_pivot <- res_learner_pivot %>%
      select(-starts_with("Learner"), -starts_with("task_id"))
    rank_matrix <- t(apply(as.data.frame(res_learner_pivot),
                           1, rank, ties.method = "average"))
    rank_dt <- as.data.table(rank_matrix)
    setnames(rank_dt, paste0(names(rank_dt), "_rank"))
    ...
    colnames(df) = c("Learner", "Calibrator", paste0(measure,"_mean"), 
                     paste0(measure,"_rank"))
    if(!exists("res_per_measure")) {
      res_per_measure <- df
    } else {
