...
learner_gam_cal <- as_learner(po("calibration", learner = learner_uncal, 
                                 method = "gam", rsmp = holdout))
learner_gam_cal$id = "Calibrated GAM (Holdout)"
...
learner_gam_cal$train(task_train)
...
preds_gam_cal = learner_gam_cal$predict(task_test)
...
brier_gam_cal = preds_iso_cal$score(msr("classif.bbrier"))
...
learner_gam = list(learner_uncal, learner_gam_cal, learner_cv_cal_gam)
learners_simple_cal = list(learner_uncal, learner_log_cal, learner_beta_cal, learner_iso_cal, learner_gam_cal)
...
calibrationplot(learner_gam, task_test, bins = 11, smooth = TRUE)
